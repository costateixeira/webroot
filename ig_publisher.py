...
class ReleasePublisher:
    ...
    def clone_repo(self, url, path, branch=None, use_sparse=False, sparse_dirs=None):
        ...
        if os.path.exists(path) and os.path.isdir(os.path.join(path, '.git')):
            ...
            try:
                repo = git.Repo(path)
                # Reset local changes (best-effort) ONLY when there is a remote to
                # sync from: reset --hard exists to clean the working tree before
                # fetch/pull from origin. For a local-only webroot (no remote, used
                # as the source of truth for publish content), this would discard
                # intentional edits (e.g. publish-setup.json), so skip it.
                ...
                if has_origin:
                    ...
                    self.log_progress(f"No 'origin' remote for {path}; keeping local changes (skipping reset --hard)")
...
"""
    ...
    def build(self):
        ...
        try:
            ...
            # Clean generated folders so every build starts fresh. The local
            # template (ig.ini: template = #<name>) is expanded into ./template
            # each build; a stale/partial copy left by a previous run makes the
            # publisher reuse it (via template/cache.ini) and then fail in
            # loadPages with "Unable to find file .../template/config".
            for gen in ('template', 'temp'):
                p = os.path.join(self.source_dir, gen)
                if os.path.isdir(p):
                    self.log_progress(f"Cleaning generated folder before build: {p}")
                    shutil.rmtree(p, ignore_errors=True)

            # Run publisher from within source directory.
            # -Dfile.encoding=UTF-8 is required: without it the JVM defaults to
            # windows-1252 on Windows, so the publisher writes the intermediate
            # template XML (onGenerate-ig-working.*.xml) with non-UTF-8 bytes,
            # and the template's XSLT (onGenerate.groupSort.xslt) then fails to
            # parse it as UTF-8 ("Invalid byte 1 of 1-byte UTF-8 sequence").
            # publish() already sets this; keep the two invocations consistent.
            self.run_command([
    ...
    def _settle_publisher(self, label=""):
        """Let the OS fully release handles/locks from a finished publisher run
        before launching the next one.

        run_command() blocks until the JVM exits, so the process is already gone;
        but Windows can lag on releasing file handles, and an interrupted build
        leaves stale '*.lock' files in the package cache (e.g.
        hl7.fhir.r4.core#4.0.1.lock) that make the next run fail with
        AccessDeniedException. Clear those and pause briefly."""
        removed = 0
        try:
            for name in os.listdir(self.package_cache):
                if name.endswith('.lock'):
                    try:
                        os.remove(os.path.join(self.package_cache, name))
                        removed += 1
                    except OSError:
                        pass
        except OSError:
            pass
        if removed:
            self.log_progress(f"Cleared {removed} stale package-cache lock(s)")
        time.sleep(5)
        if label:
            self.log_progress(f"Settled before {label}")
    ...
    def run(self):
        try:
            ...
            # Ensure the build JVM has fully released file handles/locks before
            # the second (go-publish) JVM starts.
            self._settle_publisher("go-publish")
